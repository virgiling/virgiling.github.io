...
> [!important]-
>
> 这里的推导过程较为简单，考虑 $\ket{\psi} = \alpha ket{0} + \beta \ket{1}$，由于 $\alpha, \beta in \mathbb{C}$，由[欧拉公式](https://en.wikipedia.org/wiki/Euler%27s_formula)， 我们有：
>
> $$
> \ket{\psi} = e^{i\delta} (\cos \theta \ket{0} + \sin \theta e^{i\phi}\ket{1})
> $$
>
> 其中 $e^{i\delta}$ 称作**共同相位**（global phase），因为对 $\ket{0}, \ket{1}$ 的影响都一样，无法从实验上测得，因此我们可以将其忽略，于是剩下的部分，我们就可以使用球面几何来描述，这就是布洛赫球
...
\end{bmatrix}
= \begin{bmatrix}
b_{11} & b_{12} &|& 2b_{11} & 2b_{12} &|& 3b_{11} & 3b_{12} \\
b_{21} & b_{22} &|& 2b_{21} & 2b_{22} &|& 3b_{21} & 3b_{22} \\
\hline
4b_{11} & 4b_{12} &|& 5b_{11} & 5b_{12} &|& 6b_{11} & 6b_{12} \\
