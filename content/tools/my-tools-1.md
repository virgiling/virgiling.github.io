...
最后，关于文章中的图片，我使用的是 [piclist](https://piclist.cn/) + 腾讯云对象存储，这样可以通过腾讯云的对象存储作为在线图床，就不用担心图片无法显示在公网上了

> [!important]
>
> 在 PicList 中，我们可以使用插件，因为本人使用的 `4K` 屏幕，会导致截图时图片很大，所以需要压缩，因此可以使用 [压缩插件](https://github.com/supine0703/picgo-plugin-compress-next)
> a
> 比较轻松的是 PicList 与 Picgo 的生态与接口都是兼容，可以直接从 Picgo 迁移到 PicList 中
